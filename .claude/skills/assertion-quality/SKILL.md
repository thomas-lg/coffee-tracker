...
|---------|---------|-------------|
| Trivial assertions | Test contains only `Assert.IsNotNull(result)` / `how can assert result is not None` / `expect(x).toBeDefined()` | Test passes but doesn't verify correctness |
